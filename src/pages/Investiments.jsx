...
import iconChartBar from "../svg/poll.svg";
...
function InvestModal({ inv, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl w-full max-w-md shadow-lg border border-slate-200 dark:border-slate-700 mx-4">
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100 dark:border-slate-800">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-200">
            {isEditing ? "Editar investimento" : "Novo investimento"}
          </h2>
          <button onClick={onClose} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 p-1 transition">
            <img src={iconClose} alt="" className="w-5 h-5 opacity-60" />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
          {/* Tipo */}
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Classe do ativo</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Nome do investimento</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Valor investido (R$)</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Retorno esperado (% a.a.)</label>
              <input
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Data de entrada</label>
            <input
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-200">Investimentos</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Controle e projeção da sua carteira</p>
        </div>
        <button
          onClick={() => setModal({})}
        ...
        >
          <img src={iconAdd} alt="" className="w-4 h-4" style={{ filter: "brightness(0) invert(1)" }} />
          Novo investimento
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Total investido</p>
          <p className="text-lg font-bold text-slate-800 dark:text-slate-200 truncate">{fmt(totalInvested)}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Valor estimado</p>
          <p className="text-lg font-bold text-slate-800 dark:text-slate-200 truncate">{fmt(totalCurr)}</p>
          {totalYieldNow !== 0 && (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Rentabilidade</p>
          <p className={`text-lg font-bold ${totalPct >= 0 ? "text-emerald-600" : "text-red-500"}`}>
            {totalPct >= 0 ? "+" : ""}{totalPct.toFixed(1)}%
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">desde início</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Retorno médio</p>
          <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{avgReturn.toFixed(1)}% <span className="text-xs font-normal text-slate-400 dark:text-slate-500">a.a.</span></p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{investments.length} ativo{investments.length !== 1 ? "s" : ""}</p>
        </div>
      </div>
      ...
      {investments.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">Distribuição</h3>
            {/* Segmented bar */}
            ...
            <div className="space-y-2.5">
              {distrib.map((t) => (
                <div key={t.id} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-400 flex-1 truncate">{t.label}</span>
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{fmt(t.total)}</span>
                  <span className="text-xs text-slate-400 dark:text-slate-500 w-8 text-right">{t.pct.toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 lg:col-span-2">
            <div className="mb-3">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Projeção da carteira</h3>
              <p className="text-xs text-slate-400 dark:text-slate-500">Simulação baseada nos retornos esperados de cada ativo</p>
            </div>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={portfolioChartData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: isDark ? "#64748b" : "#94a3b8" }} tickFormatter={(v) => v >= 1000 ? `${(v / 1000).toFixed(0)}K` : `${v}`} width={44} />
                  <Tooltip
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow px-3 py-2 text-xs">
                          <p className="font-medium text-slate-600 dark:text-slate-400 mb-1">{payload[0]?.payload?.label}</p>
      ...
      {investments.length === 0 ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Carteira</h3>
            <span className="text-xs text-slate-400 dark:text-slate-500">{investments.length} ativo{investments.length !== 1 ? "s" : ""}</span>
          </div>
          <div className="divide-y divide-slate-50 dark:divide-slate-700">
            {analyzed.map((inv) => {
              ...
              return (
                <div key={inv.id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                  {/* Type icon */}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">{inv.name}</p>
                  ...
                  <div className="hidden sm:flex items-center gap-6 text-right">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{fmt(inv.amt)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-400 dark:text-slate-500">Atual est.</p>
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{fmt(inv.curr)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-400 dark:text-slate-500">Rendimento</p>
      ...
      {investments.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
            <div>
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Comparação com CDI</h3>
              <p className="text-xs text-slate-400 dark:text-slate-500">Veja como cada ativo se compara ao benchmark</p>
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs text-slate-500 dark:text-slate-400 flex-shrink-0">CDI (% a.a.):</label>
              <input
                type="number" value={cdiFee} onChange={(e) => setCdiFee(e.target.value)}
                className="border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-sm w-20 bg-white dark:bg-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500"
                step="0.01"
              />
            </div>
          </div>
                ...
                <div key={inv.id}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">{inv.name}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-400 dark:text-slate-500">{inv.cdiPct.toFixed(0)}%</span>
                    </div>
                  </div>
          ...
          {(() => {
            ...
            if (below === 0) return <p className="text-xs text-emerald-600 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">Todos os ativos superam o CDI.</p>;
            if (below === analyzed.length) return <p className="text-xs text-amber-600 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">Todos os ativos estão abaixo do CDI. Considere renda fixa pós-fixada.</p>;
            return <p className="text-xs text-slate-500 dark:text-slate-400 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">{below} ativo{below !== 1 ? "s" : ""} abaixo do CDI. Avalie realocar para ativos mais rentáveis.</p>;
          })()}
        </div>
      )}
      ...
      {analyzed.length > 0 && (() => {
        ...
        return (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">Projeções de longo prazo</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {items.map((item) => (
                <div key={item.label} className={`rounded-xl border p-3.5 ${item.green ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700"}`}>
                  <p className={`text-xs mb-1 ${item.green ? "text-emerald-600" : "text-slate-400 dark:text-slate-500"}`}>{item.label}</p>
                  <p className={`text-base font-bold truncate ${item.green ? "text-emerald-700" : "text-slate-800 dark:text-slate-200"}`}>{fmt(item.value)}</p>
                  {item.sub !== 0 && (
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
        <div className="mb-4">
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Simular investimento</h3>
          <p className="text-xs text-slate-400 dark:text-slate-500">Projete o crescimento de qualquer valor com juros compostos</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            ...
            <div key={f.label}>
              <label className="block text-xs text-slate-400 dark:text-slate-500 mb-1.5">{f.label}</label>
              <input
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3.5">
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Total investido</p>
            <p className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate">{fmt(simInvested)}</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3.5">
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Valor final</p>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{fmt(simTotal)}</p>
          </div>
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5">
            <p className="text-xs text-emerald-600 mb-1">Juros compostos</p>
            <p className="text-sm font-bold text-emerald-700 truncate">{fmt(simYield_)}</p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[2px] z-50">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl w-full max-w-sm mx-4 text-center border border-slate-200 dark:border-slate-700 shadow-lg">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => setDeleteId(null)} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition">Cancelar</button>
